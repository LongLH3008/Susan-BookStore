...
class OrderService {
  ...
  static async createOrder(data: CreateOrderInputDTO) {
    ...
    if (userId) {
      ...
      if (code) {
        ...
        if (!foundCode)
          throw new ResourceNotFoundError("ma giam gia khogn ton tai ton tai");
    ...
    if (!userId && code)
      throw new ResourceNotFoundError("ban can dang nhap de suu dung code");
  ...
  // detail order
  static async DetailOrder(id: string): Promise<detailOrderData> {
    try {
      // Tìm order theo id
      ...
      order.user_avatar =
        userMap.get(order.userId.toString())?.user_avatar || "";
  ...
  static async Updateiscomment(
    ...
    const product = checkorder.products.find(
      (item) => item.bookId.toString() === bookId.toString()
    );
    if (product) {
      product.isComment = iscomment; // Cập nhật isComment
        await checkorder.save();
      console.log("Updated successfully");
    } else {
      throw new Error("Product with the given bookId not found");
    }
