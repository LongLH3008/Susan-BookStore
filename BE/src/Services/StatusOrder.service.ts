...
class StatusOrerService {
    ...
    static async UpdateStatusOrderForAdmin(id: string, state: string) {
        try{
            // checkId Order
           const validStatuses = ["pending", "confirmed", "shipped", "cancelled", "success"];
           if(!validStatuses.includes(state)) {
            throw new Error("Invalid status. Valid statuses: pending, confirmed, shipped, cancelled");
           }
           // udpate 
           const udpateOrder = await Order.findByIdAndUpdate(id, {state : state}, {new: true});
           if(!udpateOrder) {
