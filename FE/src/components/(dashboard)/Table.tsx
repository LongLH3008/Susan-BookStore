...
const Table = (props: any) => {
	...
	for (let index = 0; index < props?.total_pages; index++) {
		elements.push(
			<li key={index}>
				<Link
					to={`/${props?.header?.path}?page=${+index + 1}`}
					className="flex items-center justify-center px-3 h-8 text-blue-600 border border-gray-300  hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white"
				>
					{index + 1}
				</Link>
			</li>
			...
			<table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
				<thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
					<tr>
						...
						{props?.header?.attributes?.map((e: any) => (
					...
					{props?.data?.map((e: any) => (
						<tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
								...
								>
									{e?.attribute && (
										<div className="relative group">
											...
											<div
												id={e?.attribute?.key}
												className="z-10 absolute bg-white divide-x divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 hidden group-hover:block bottom-full -left-1"
											>
												<table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
													<thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
														<tr>
							...
							<td className="px-6 py-4">
								<Link
									to={`/${props?.header?.path}/edit/${e?._id}`}
			...
			>
				<span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">
					...
					<span className="font-semibold text-gray-900 dark:text-white">
						1-{props?.data?.length} of {props?.total}
					</span>
				</span>
				<ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
					<li>
						<Link
							to={`/${props?.header?.path}?page=${
								props?.current_page > 1 ? +props?.current_page - 1 : 1
							}`}
