	...
	return (
		<>
			<div className="relative ">
				...
				>
					{DataBanners.data?.metadata?.data.map((data: IBannerHome, index: number) => (
						<SwiperSlide
							key={index}
						...
						>
							...
							>
								...
								<p className="sm:w-[650px] min-[320px]:w-[70%]">{data.description}</p>
								<Link
									to={data.url}
									className="lg:text-2xl sm:text-xl font-semibold border-4 border-[#333333] lg:px-10 sm:px-6 min-[320px]:px-4 lg:py-5 sm:py-4 min-[320px]:py-2 rounded-full my-10 hover:bg-[#333333] hover:text-white animate-bounce transition-transform duration-1500 transform translate-y-[-20px] delay-700"
								>
									Shop Now
								</Link>
							</div>
