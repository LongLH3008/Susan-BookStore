...
const ItemTotal = ({ data }: { data: ICart }) => {
	const { price, discount, title } = data.product_id;
	...
	return (
		<div className="relative w-full grid grid-cols-4 items-start">
			...
			<div className="text-[14px] text-zinc-700 flex items-center justify-end gap-x-1">
				{discount == 0 ? (
					<>
						<span className="mr-1 line-through text-zinc-500 text-transparent">${price}</span>
						<span>${price}</span>
					</>
				) : (
					<>
			...
			<p className="ml-[2%] text-[15px] justify-end flex">${totalPriceItem}</p>
		</div>
	);
};

export default function TotalCart({ dataCart, user_id }: { dataCart: ICart[]; user_id: string }) {
	console.log(dataCart);
	const subtotal = dataCart.reduce(
		(acc: number, item: any) => acc + item.product_id.price * item.product_quantity,
		0
	);
	const discountArr = dataCart.filter((item: any) => item.product_id.discount > 0 && item);
