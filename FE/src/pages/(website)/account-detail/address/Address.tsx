...
import { IUserAddress } from "@/common/interfaces/user";
				...
				<div className="flex flex-col gap-4 pb-5">
					{data && data?.metadata?.data.length > 0 ? (
						data?.metadata.data.map((item: IUserAddress, index: number) => (
							<div
								key={index}
								className="rounded-md p-5 flex items-center justify-between border border-zinc-200 hover:bg-zinc-100 cursor-pointer"
							>
								...
								<div className="flex items-center gap-3 text-[12px]">
									<UpdateAddress
										dataAddress={item}
										trigger={<span className="hover:underline">Cập nhật</span>}
									/>
									<span
						...
						<div className="h-[50dvh] text-sm flex items-center justify-center text-zinc-400">
