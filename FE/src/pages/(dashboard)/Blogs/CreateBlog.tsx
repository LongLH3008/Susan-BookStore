...
const CreateBlog = () => {
	...
	const { mutateAsync } = !id
		...
		: useMutation({
				mutationFn: ({ data, id }: { data: IBlog; id: string }) => updateBlog(data, id),
				onSuccess: (data: any) => {
					nav("/quan-tri/san-pham");
					toast({
						variant: data.status,
		...
		<>
			<div className="p-5 flex justify-between items-center bg-white shadow-sm rounded-lg">
				<div className="flex items-center gap-3">
					...
					<h2 className={`text-xl font-[500]`}>{!id ? "Thêm" : "Sửa"} tin tức</h2>
				</div>
					...
					<div className="w-full col-span-3 *:mb-6">
						<div>
							...
							<input
								...
								placeholder="John"
								{...register("blog_title")}
								defaultValue={blog?.metadata?.blog_title && blog.metadata.blog_title}
							/>
							{errors.blog_title && (
					...
					<div className="col-span-1 *:mb-6">
						...
						<div>
							...
							<input
								...
								placeholder="John"
								defaultValue={blog?.metadata?.blog_author && blog.metadata.blog_author}
