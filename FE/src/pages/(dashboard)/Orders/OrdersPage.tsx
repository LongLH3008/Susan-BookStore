...
const OrdersPage = () => {
          ...
          <select
            ...
            style={{
              backgroundColor: statusList.find(
                (item) => item.label == params.row.state
              )?.bg,
              color: statusList.find((item) => item.label == params.row.state)
                ?.color,
            }}
  ...
  const rows = DataOrders.data.metadata.data.map(
    (row: IOrder, index: number) => ({
      id: row._id,
      ...row,
      stt: index + 1,
    })
