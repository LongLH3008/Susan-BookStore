...
import { UpdateStatusBanners } from "@/services/banner.service";
import CloseIcon from "@mui/icons-material/Close";
import InfoIcon from "@mui/icons-material/Info";
import { CircularProgress, Dialog, DialogContent, DialogTitle, IconButton, Tooltip, Typography } from "@mui/material";
...
const BannerPage = () => {
	...
	const [selectedBanner, setSelectedBanner] = useState<IBannerHome | null>(null);
	...
	const columns: GridColDef[] = [
		{
			...
			renderCell: (params) => (
				<img src={params.value} alt={params.row.title} className="h-20 object-cover" />
			),
			width: 200,
		},
		{
	...
	const rows = DataBanners.data.metadata.data.map((row: IBannerHome, index: number) => ({
		id: index,
		...row,
	}));
