...
import { CreateBanners, getBannersByID, UpdateBanners } from "@/services/banner.service";
import { joiResolver } from "@hookform/resolvers/joi";
import NavigationIcon from "@mui/icons-material/Navigation";
import { Backdrop, Button, CircularProgress, FormControlLabel, Radio, RadioGroup } from "@mui/material";
...
const CreateBannerHome = () => {
	...
	const { mutateAsync, isPending } = !id
		...
		: useMutation({
				mutationFn: ({ data, id }: { data: IBannerHome; id: string }) => UpdateBanners(id, data),
				onSuccess: (data: any) => {
					nav("/quan-tri/anh-quang-cao");
					DataBanners.refetch();
					toast({
		...
		<>
			<div className="p-5 flex justify-between items-center bg-white shadow-sm rounded-lg mb-[50px]">
				<div className="flex items-center gap-3">
					...
					<h2 className={`text-xl font-[500]`}>{!id ? "Thêm" : "Sửa"} Ảnh banner</h2>
				</div>
						...
						<div className="w-full col-span-3 *:mb-5">
							...
							<div>
								<label
									...
									className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
								>
									Link điều hướng :<span className="text-sm text-red-500">*</span>
								</label>
