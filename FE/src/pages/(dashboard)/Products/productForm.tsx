...
import { Autocomplete, Box, Button, FormControl, FormLabel, TextField } from "@mui/material";
...
const ProductForm: React.FC = () => {
	...
	const onSubmit = async (data: Book) => {
		try {
			...
			if (selectedCoverImage) {
				...
				const coverImageResponse = await axios.post("http://localhost:5000/api/v1/upload", coverImageFormData);
	...
	useEffect(() => {
		if (book && !isReset) {
			reset({
				...book.metadata,
				publicationDate: book.metadata.publicationDate ? book.metadata.publicationDate.split("T")[0] : "",
			});
	...
	return (
		<>
			<Box className="flex items-center justify-center h-48 mb-4 rounded bg-gray-50 dark:bg-gray-800">
				<p className="text-2xl font-bold text-gray-800 dark:text-gray-50">{id ? "Chỉnh sửa sản phẩm" : "Thêm mới sản phẩm"}</p>
			</Box>

			<Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate autoComplete="off" p={2}>
					...
					<Controller
						...
						render={({ field }) => (
							<Autocomplete
								...
								value={formats.find((option) => option.label === field.value) || null}
							/>
						)}
					/>
				</FormControl>
				...
				<Box mt={2}>
					...
					{previewCoverImage && <img src={previewCoverImage} alt="Cover Preview" width="200" />}
