	...
	const columns = React.useMemo(
		() => [
			...
			},
			{
				...
				field: "discount_type",
				flex: 3,
				renderCell: (arg: any) => <span>{handleTypes(arg.row.discount_type)}</span>,
			},
			{
				headerName: "Áp dụng",
				field: "discount_applies_to",
				flex: 3,
				renderCell: (arg: any) => <span>{handleApplies(arg.row.discount_applies_to)}</span>,
			},
			{
				headerName: "Số lượng",
			...
			},
			{
				...
				renderCell: (arg: any) => <span>{arg.row.discount_users_used.length}</span>,
			},
			{
				headerName: "Ngày bắt đầu",
				field: "discount_start_date",
				flex: 3,
				renderCell: (arg: any) => <span>{handleDate(arg.row.discount_start_date)}</span>,
			},
			{
				headerName: "Ngày kết thúc",
				field: "discount_end_date",
				flex: 3,
				renderCell: (arg: any) => <span>{handleDate(arg.row.discount_end_date)}</span>,
			},
			{
				headerName: "Trạng thái",
				field: "discount_is_active",
				renderCell: (arg: any) => (
					<span>{arg.row.discount_is_active ? "Kích hoạt" : "Chưa kích hoạt"}</span>
				),
			},
			{
