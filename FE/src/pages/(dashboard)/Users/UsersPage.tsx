...
const UsersPage: React.FC = () => {
	...
	const { mutate } = useMutation({
		mutationFn: ({ id, payload }: { id: string; payload: { user_status: string } }) =>
	...
	const columns = React.useMemo(
		() => [
				...
				renderCell: (params: any) => (
					<div className="flex items-center space-x-2">
						{params.row.user_avatar ? (
							...
							<Avatar className="bg-green-900">{getInitials(params.row.user_name)}</Avatar>
						)}
			...
			},
			{
				...
				field: "user_role",
				renderCell: (params: any) => (
					<p>{params.row.user_role == "user" ? "Khách hàng" : "Quản trị viên"}</p>
				),
				flex: 5,
			},
			{
				...
				renderCell: (params: any) => (
					<>
						...
						{params.row.user_status !== "active" && params.row.user_role === "user" && (
	...
	return (
		<div>
			...
			<MyTable2 rows={data?.metadata?.allUsers || []} columns={columns} loading={isLoading} />
			{isError && (
				<div
					style={{
						display: "flex",
				...
				<DialogContent dividers>
					{selectedUser ? (
						<Box>
							...
							<Typography>
								<strong>Địa chỉ:</strong> {selectedUser.user_address || "Chưa cập nhật"}
							</Typography>
							<Typography>
